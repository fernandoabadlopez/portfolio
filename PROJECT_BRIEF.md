...
Owned by `DESIGN.md`, generated by Impeccable at the end of the redesign. The owner reopened the visual direction completely: the earlier brutalist / Technor / Switzer notes are retired and nothing visual is pinned by this brief. Product truth for design tooling lives in `PRODUCT.md`, which defers to this brief for every rule below.
...
Motion, page transitions and live canvas simulations on the site itself are allowed, provided they honour `prefers-reduced-motion` and add no new dependencies (native CSS, `astro:transitions`, hand-written TypeScript).